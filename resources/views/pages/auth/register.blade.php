...
class extends Component
{
    ...
    /** 1 = method, 2 = details/otp, 3 = phone verify/profile */
    public int $step = 1;
    ...
    public function register(): void
    {
        ...
        try {
            ...
            if ($phone && $this->signup_method === 'email') {
    ...
    public function verifyPhone(): void
    {
        ...
        try {
            if ($this->signup_method === 'phone') {
                $response = $this->api->auth()->verifyOtp($this->phone_code, $this->login_token);
...
<div class="flex h-full flex-col overflow-hidden bg-cream px-5 pb-6 pt-2">
    ...
    @if ($step === 1)
        {{-- Heading --}}
        <h1 class="mb-1 mt-4 font-heading text-[24px] font-[800] leading-tight text-bark">{{ __('auth.create_your_account') }}</h1>
        ...
        @if ($phoneEnabled)
            ...
            <div>
                ...
                <form wire:submit="sendPhoneCode" class="flex gap-2">
    ...
    {{-- Step 2: OTP (phone) or Details (email) --}}
    @elseif ($step === 2)

        @if ($signup_method === 'phone')
            {{-- Phone OTP verification (matches login OTP screen) --}}
            ...
            <p class="mb-6 text-[13px] font-bold text-bark">{{ $phone_number }}</p>
    ...
    {{-- Step 3: Profile completion (phone) or Phone verification (email) --}}
    @elseif ($step === 3)
        ...
        @if ($signup_method === 'phone')
            ...
            <h1 class="mb-1 font-heading text-[20px] font-[800] leading-tight text-bark">{{ __('auth.your_details') }}</h1>
            <p class="mb-5 text-[12px] text-muted">{{ __('auth.complete_your_profile') }}</p>

            <form wire:submit="completeProfile" class="flex flex-1 flex-col gap-2.5">
                {{-- First + Last name --}}
                <div class="flex gap-2">
                    <div class="flex-1">
                        <label class="mb-1.5 block text-[10px] font-bold uppercase tracking-wide text-muted">{{ __('auth.first_name') }}</label>
        ...
        @else
            {{-- Email signup: Phone verification --}}
