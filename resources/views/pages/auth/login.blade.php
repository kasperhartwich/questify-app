...
class extends Component
{
    ...
    public bool $testerEnabled = false;
    ...
    public function mount(): void
    {
        ...
        $this->testerEnabled = $appInfo->isAuthMethodEnabled('tester');
    }
    ...
    {
        // Defence in depth: a stale view must not be able to reach the tester
        // endpoint once the backend stops offering it.
        if (! app(AppInfoService::class)->isAuthMethodEnabled('tester')) {
            return;
        }
...
<div class="flex h-full flex-col overflow-hidden bg-cream px-5 pb-6 pt-2">
    @if ($step === 'login')
        ...
        @if (count($socialProviders) > 0)
            <div class="mb-[4px] grid grid-cols-2 gap-[8px]">
                @foreach ($socialProviders as $provider)
                    <a href="{{ app(\App\Services\AppInfoService::class)->socialRedirectUrl($provider) }}" class="flex w-full items-center justify-center gap-[8px] rounded-[14px] border-[1.5px] border-cream-border bg-transparent py-[13px] text-[14px] font-semibold text-bark">
                        @if ($provider === 'google')
        ...
        {{-- Tester login (backend-gated: hidden unless TESTER_CLIENT_ID is set) --}}
        @if ($testerEnabled)
            ...
            <form wire:submit="login">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="email" wire:model.live="email" name="email" autocomplete="email" placeholder="{{ __('auth.email_placeholder') }}" class="w-full rounded-[14px] border-[1.5px] border-cream-border bg-white py-[13px] pl-9 pr-3.5 text-[13px] text-bark placeholder:text-forest-300 focus:border-forest-600 focus:outline-none" required />
                    </div>
                ...
                @if (strlen($email) > 0)
                    <div class="mt-2.5">
                        ...
                        <div class="relative">
                            ...
                            <input type="password" wire:model="password" name="password" autocomplete="current-password" placeholder="{{ __('general.password') }}" class="w-full rounded-[14px] border-[1.5px] border-cream-border bg-white py-[13px] pl-9 pr-3.5 text-[13px] text-bark placeholder:text-forest-300 focus:border-forest-600 focus:outline-none" required />
                        </div>
